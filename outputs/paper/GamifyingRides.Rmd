...
# Shiny App Preview
```{r Shiny App, fig.cap="Figure 1. Shiny App Preview", echo = FALSE}
...
```{r Table2, fig.cap="Overview of User 2 Dataset", echo = FALSE}
user1 <- read.csv("https://raw.githubusercontent.com/edenbarker/peloton_dataset/main/Peloton%20User%201.csv")

user1 |>
  select(date, length_minutes, type, total_output) |>
  slice(131:155) |> 
  knitr::kable(
    caption = "Dataset Overview of User 1 Workouts in April 2021",
    col.names = c("Date", "Length in Minutes", "Class Type", "Total Output"),
    digits = 1,
    booktabs = TRUE,
    linesep = "",)
```
...
#Get dataset
user2 <- read.csv("https://raw.githubusercontent.com/edenbarker/peloton_dataset/main/Peloton%20User%202.csv")
...
user2df |>
    ggplot(mapping = aes(x = date, fill = total_output)) +
  geom_bar() +
  theme_minimal() +
  labs(x = "Workout Dates",
       y = "type",
       fill = "Total Output") +
  facet_wrap(vars(length_minutes),
             dir = "v",
             scales = "free")
```
